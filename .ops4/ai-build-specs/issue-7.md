...
## Objective
Enable all documented routes in the Next.js App Router so that every page and modal listed in the platform's screen map loads the correct screen/component instead of a 404 error. Ensure each route renders its corresponding page/component, including all required state variants (empty, loading, error, plan_limit, permission_denied) as described in the design and data point documentation. Maintain all existing layouts, navigation, and design system patterns without altering the sidebar, topbar, or card/table styles.
...
## Requirements
- Add or update Next.js App Router route files and folders to match every documented page and modal in the screen map (e.g., /dashboard, /briefs, /doctors, /matching, /outreach, /follow-ups, /reporting, /admin, and all nested/modal/drawer routes).
- Ensure each route renders the correct screen/component, including all state variants (empty, loading, error, plan_limit, permission_denied) as specified in the Design Brain and Page-by-page Data Points.
- Use only existing component patterns, layouts, and design system styles for all new or updated routes; do not modify the sidebar, topbar, card/table styles, or any existing UI patterns.
- Implement all required fields and filters for each page as listed in the Page-by-page Data Points summary.
- Do not remove or redesign any existing pages, navigation, or layouts; this patch is routing-only.
- Maintain design continuity and incremental change—no full UI rewrites.
...
## Acceptance Criteria
- All URLs listed in the screen map (including nested and modal/drawer routes) load the correct page/component and do not return a 404 error.
- Each route displays the correct state variants (empty, loading, error, plan_limit, permission_denied) as per design requirements.
- All new or updated routes use the existing design system and component patterns without introducing new layouts or styles.
- All required fields and filters for each page are present and match the documented data points.
- Lint passes
- Build passes
...
`ai-build/issue-7-implement-full-app-router-routing-for-all-docume`
...
PR title should be:
AI Build: Implement Full App Router Routing for All Documented Pages
