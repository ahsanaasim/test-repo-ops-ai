...
## Objective
Build the initial application using the locked workspace dashboard design system. Implement the full app shell (sidebar, top bar, responsive layout) and all listed screens, modals, and drawers with their documented fields, filters, and states. Use Next.js (App Router), TypeScript, Tailwind CSS, and shadcn/ui, ensuring all UI matches the specified design system and page-by-page data points.
...
## Requirements
- Set up Next.js (App Router) project with TypeScript, Tailwind CSS, and shadcn/ui (New York) components.
- Implement persistent, collapsible left sidebar (icon+label, role-aware nav: Dashboard, Briefs, Doctors, Matching, Outreach, Follow-Ups, Reporting, Admin).
- Add slim top bar with breadcrumb/page title (left), search (Command), notifications (Sheet/Drawer), and user menu (DropdownMenu, right).
- Apply locked design system: color, typography, spacing, radius, elevation, component usage, and motion as specified.
- Implement all listed pages, modals, and drawers with all documented fields, filters, and states (empty, loading, error, permission_denied, plan_limit) using realistic sample data.
- Ensure all tables use shadcn/ui Table with sticky headers, hairline dividers, comfortable row height, and no zebra stripes.
- Apply max-w-7xl container for main content, with p-8 desktop and p-4 mobile/tablet padding.
- Implement role-based navigation and data masking per requirements (e.g., Reporting User restrictions).
- Ensure all interactive elements are accessible (WCAG 2.1 AA), keyboard navigable, and have visible focus rings.
- Do not include any billing, invoicing, payroll, or revenue features.
- Maintain design continuity: no deviation from locked design system or page structure.
...
## Acceptance Criteria
- All listed screens, modals, and drawers are implemented with correct fields, filters, and states as per page-by-page data points.
- Sidebar and top bar match locked design and are fully functional and responsive.
- All tables, cards, badges, tabs, dialogs, sheets, selects, and inputs use shadcn/ui components and follow design system rules.
- Role-based navigation and data masking are enforced as per requirements.
- Accessibility: keyboard navigation, visible focus, and color contrast meet WCAG 2.1 AA.
- No commercial/financial features are present.
- Lint passes
- Build passes
...
`ai-build/issue-3-implement-locked-design-baseline-app-shell-navig`
...
PR title should be:
AI Build: Implement locked design baseline: app shell, navigation, and all core screens
