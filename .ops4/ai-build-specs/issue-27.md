...
## Objective
Update the /login and /register pages to use the latest modern, minimal authentication UI patterns as specified in the locked Design Brain prompt. Preserve all existing layouts, navigation, and design system elements elsewhere in the app.

## Context
## decision:approved
- id=44b47331-dd8a-458d-bf6a-14f49850761a title="Design patch: Workspace Dashboard" confidence=n/a description="Update our login and registration page UI. Current one is outdated."
- id=2a97ee92-f65c-4486-9f60-8fb701ff26b6 title="Design patch: Workspace Dashboard" confidence=n/a description="Update our authentication flow and screens."
...
- id=083524b9-8795-4e6f-8675-bad80dc654e0 title="Recruiter dashboard" confidence=1.00 description="Explicitly listed as an MVP feature, likely the main landing screen for recruiters."
## user_role:approved
...
Purpose: Add internal verification notes.
Layout: **single-column-form**
...
Purpose: Edit follow-up task details.
Layout: **single-column-form**
...
Route: `/register`
Purpose: Allow new internal users to register for access, using the same form patterns as login.
Layout: **two-column-main-rail**

## Required regions
- **main**: Allow new internal users to register for access, using the same form patterns as login. — components: PageHeader, PrimaryContent
- **context-rail**: Related context, history, or secondary actions — components: RelatedInfo, ActivityOrMeta
...
## Requirements
- Implement the /login and /register pages using shadcn/ui Card with shadow-sm, rounded-lg, and responsive padding (p-8 desktop, p-4 mobile/tablet).
- For /login: Include Email (Input[type=email]), Password (Input[type=password]), Show Password Toggle, Login Button (primary), and Forgot Password Link (secondary/ghost). Support error, loading, plan_limit, and permission_denied states using Alert, Banner, or Spinner as per the design system.
- For /register: Use the same Card and form layout as /login. Include Email, Password, Confirm Password, Show Password Toggle, Register Button (primary), and a link to Login (secondary/ghost). Support error, loading, plan_limit, and permission_denied states, and show password requirements Tooltip as needed.
- Ensure both pages are fully responsive, accessible (WCAG AA), and use only shadcn/ui components and patterns as specified.
- Do not modify or redesign any other pages, navigation, sidebar, or theme elements.

## Design Continuity Rules
- Apply the requested change, including Design Pattern Brain composition for matching screens.
- Preserve product brand tokens (logo, primary colors from locked design system).
- For authentication/register/login/landing screens covered by retrieved patterns, prefer Pattern Brain composition over the previous layout.
- Reuse existing components and coding conventions where they do not conflict with the required pattern composition.
- Do not clone proprietary branding or copy from pattern source products.
...
## Acceptance Criteria
- /login and /register pages match the locked Design Brain authentication UI patterns and component usage.
- All specified fields, states, and interactions are implemented and visually consistent with the design system.
- Pages are responsive and accessible (WCAG AA), with correct keyboard navigation and focus states.
- No changes to dashboard, navigation, sidebar, or other existing pages.
- Lint passes
- Build passes
...
- Workspace screens match `.ops4/design-system/screens/` blueprint regions; auth/register/login may follow Design Pattern Brain instead when those briefs are present.

## Agent Instructions
Create a new branch named:
`ai-build/issue-27-update-login-and-registration-pages-to-modern-au`
...
PR title should be:
AI Build: Update Login and Registration Pages to Modern Authentication UI
...
Apply the requested change, including restyling screens using Design Pattern Brain when briefs are attached.
Open `.ops4/design-system/patterns/` and reshape matching screens (auth/register/login/landing) to the retrieved composition archetypes.
Keep product brand colors/logo from the locked design system; adapt spacing, hierarchy, density, and structure from Pattern Brain.
...
When screen blueprints are provided for workspace screens, do not collapse required regions.
Pattern Brain is required for matching screen types in this change — not optional inspiration.