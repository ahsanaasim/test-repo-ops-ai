...
## Objective
Review all Workspace Dashboard screens and states to ensure strict adherence to the 14 Workspace Dashboard UI/UX guidelines. Incrementally patch any missing filters, badges, status variants, or field-level density issues using the existing shadcn/ui and lucide-react component patterns. Do not alter layout, navigation, or theme beyond what is required for compliance.
...
## Requirements
- Audit every screen and state in the Workspace Dashboard for compliance with the 14-point guidelines (sidebar, top bar, content container, cards, tables, filters, modals, tabs, accessibility, states, accent color usage, motion, responsiveness).
- For each page, cross-check against the Page-by-page data points summary to ensure all documented filters, badges, status variants, and fields are present and visible (including all filters above tables/lists, all status badges, and all edge case states).
- If any required filter, badge, status variant, or field is missing, add it using shadcn/ui and lucide-react components, matching the existing design system (no new layouts or themes).
- Ensure all interactive elements have visible focus rings and are fully keyboard navigable (WCAG AA).
- Do not redesign, remove, or add navigation items, layouts, or visual themes except as required for strict guideline compliance.
- Verify all listed pages and routes are implemented and accessible (no 404s for valid routes).
- Maintain all existing design system constraints: color, typography, spacing, card/table/modal patterns, and responsive breakpoints.
...
## Acceptance Criteria
- All Workspace Dashboard screens and states strictly follow the 14-point guidelines.
- All documented filters, badges, status variants, and fields from the Page-by-page data points are present and visible.
- No edge-to-edge text or non-compliant accent color usage is present.
- All interactive elements have visible focus rings and pass keyboard navigation checks.
- No navigation, layout, or theme changes beyond what is required for compliance.
- All valid routes are accessible and do not return 404 errors.
- Lint passes
- Build passes
- PR opened to default branch
- UI follows the 14 Aperture guideline docs under `.ops4/design-system/` (README conflict priority wins); no inventing a new palette or shell.

## Agent Instructions
Create a new branch named:
`ai-build/issue-15-audit-and-patch-workspace-dashboard-ui-for-14-po`
...
PR title should be:
AI Build: Audit and Patch Workspace Dashboard UI for 14-Point Guideline Compliance
