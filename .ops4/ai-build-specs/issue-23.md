...
## Objective
Update every screen and modal to fully surface all required data points, filters, columns, badges, and status variants as specified in the latest Page by Page Data Points and design guidelines. Follow the locked Design Brain baseline and do not alter existing layouts, navigation, or component patterns.
...
- Display all table columns, badges, and meta fields as specified per page, including AI match confidence, AHPRA status, assignment, etc.
- Support all status variants (empty, loading, error, plan_limit, permission_denied) with correct illustration, messaging, and CTA per data points.
- Apply all role-based visibility and data masking rules as described in the data points and business rules.
- Do not change layout, styling, navigation, or component patterns—only enrich each screen to surface all required data points, filters, and edge case states.
- Use only existing shadcn/ui component patterns from the Design Brain baseline.
...
## Acceptance Criteria
- All screens and modals display the full set of required data points, filters, columns, badges, and status variants per the latest Page by Page Data Points.
- Role-based visibility and data masking rules are enforced as specified.
- No changes to layout, navigation, or component structure outside of enriching with new data points and filters.
- All new and updated UI elements use shadcn/ui components and follow the locked design system.
- Lint passes
- Build passes
...
`ai-build/issue-23-enrich-all-screens-with-updated-data-points-and-`
