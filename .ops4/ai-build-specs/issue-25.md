...
## Objective
Implement the latest design patch by adding a public-facing Landing Page at route `/` and a Registration Page at `/register`, following the locked Design Brain prompt and existing design system. Preserve all existing layouts, navigation, and component patterns for authentication and registration flows.
...
## Requirements
- Add a Landing Page at `/` with headline, description, and primary actions for Login and Register, using the existing card and button components, centered with no sidebar or topbar.
- Add a Registration Page at `/register` using the same single-column card layout, form, and alert patterns as the login screen, with fields for name, email, password, and confirm password.
- Implement error, loading, plan_limit, and permission_denied states on the Registration Page using the same Alert and Banner components as login.
- Provide a link from the Registration Page to return to the Login Page.
- Do not modify or redesign any other pages, layouts, or navigation; use only existing component patterns from the locked design system.
...
- Open `.ops4/design-system/patterns/` for retrieved composition patterns (secondary to locked prompt + blueprints).

## Acceptance Criteria
- Landing Page and Registration Page match the locked design system and component patterns.
- All specified states (error, loading, plan_limit, permission_denied) are implemented for Registration.
- Navigation between Landing, Login, and Registration pages works as described.
- No changes to dashboard, navigation, or other existing pages.
- Lint passes
- Build passes
...
- Every screen matches its `.ops4/design-system/screens/` blueprint regions; no collapsed single-card layouts where multi-region is specified.
- Composition taste follows retrieved design pattern briefs under `.ops4/design-system/patterns/` without copying proprietary branding.

## Agent Instructions
Create a new branch named:
`ai-build/issue-25-add-landing-and-registration-pages-to-authentica`
...
PR title should be:
AI Build: Add Landing and Registration Pages to Authentication Flow
...
When design pattern briefs are provided, use them for composition taste only.