...
## Objective
Update all screens, states, and components to strictly follow the locked Workspace Dashboard design system and the 14 Workspace Dashboard guidelines. This is an incremental visual and component update only: do not change layout, navigation, or add/remove screens. Ensure all UI elements, typography, spacing, and interactive states match the specified design system, using shadcn/ui and lucide-react components throughout.
...
Fourteen Aperture UI/UX guideline documents are included — follow them over generic aesthetics (see README conflict priority).
...
Guideline files in the repo under `.ops4/design-system/`:
...
## Requirements
- Update all pages, modals, drawers, and components to use the locked Workspace Dashboard design system: persistent collapsible left sidebar (icon + label, role-aware), slim top bar (breadcrumb/title left, search + user menu right), max-w-7xl container, responsive breakpoints.
- Apply correct color palette: neutral base (zinc/slate), accent indigo/violet (hsl(243 75% 59%)) for primary actions, bg-card with border-border, no gradients or rainbow palettes.
- Enforce typography: Inter or Geist, page titles text-2xl font-semibold, section headings text-base or text-lg font-medium, body text-sm, meta/labels text-xs text-muted-foreground uppercase tracking-wide.
- Update all cards, tables, widgets, and filters to use shadcn/ui (New York) + Tailwind: rounded-lg (cards/inputs), rounded-md (buttons), shadow-sm (cards), shadow-md (dialogs), correct padding and spacing (8px scale).
- Ensure all data tables use sticky headers, hairline row dividers, no zebra stripes, comfortable row height, and always-visible filters above tables/lists using shadcn/ui Select/Input.
- Update all status variants (empty, loading, error, plan_limit, permission_denied) to use correct illustration/icon, Skeleton, Alert, or banner as per the design system.
- All interactive elements must have visible focus-visible rings and full keyboard navigation (WCAG AA).
- All modals/dialogs must use shadcn/ui Dialog with rounded-lg, shadow-md, and correct padding; all drawers/sheets must use shadcn/ui Sheet with correct styling.
- All buttons must use the correct style (primary, secondary, ghost) with rounded-md and focus-visible ring.
- All icons must use lucide-react, stroke only, correct sizing (20-24px for nav/actions, 32-48px for empty/illustration).
- All pages must remain responsive: sidebar collapses to drawer and content stacks vertically on mobile/tablet.
- Do not change layout, navigation, add/remove screens, or alter page structure; only update UI/UX to match the locked design system.
- Implement all fields, filters, and status variants as listed in the Page-by-page data points summary for each screen.
...
- Base branch: main
- PR target: main
- Open and follow `.ops4/design-system/README.md` plus `01`–`14` guideline markdown files before inventing UI patterns.

## Acceptance Criteria
- All screens, components, and states visually match the locked Workspace Dashboard design system and guidelines.
- All data tables, filters, cards, modals, drawers, and buttons use shadcn/ui and lucide-react as specified.
- All status variants (empty, loading, error, plan_limit, permission_denied) are present and styled per the design system.
- All interactive elements are accessible (focus-visible, keyboard navigation, WCAG AA contrast).
- No layout, navigation, or page structure changes are introduced.
- All required fields and filters from the Page-by-page data points summary are present and styled correctly.
- Lint passes
- Build passes
...
`ai-build/issue-16-update-ui-to-locked-workspace-dashboard-design-s`
...
PR title should be:
AI Build: Update UI to Locked Workspace Dashboard Design System
...
When the 14 design guideline documents are provided, treat them as the visual and UX source of truth over any generic aesthetic. Respect README conflict priority.
