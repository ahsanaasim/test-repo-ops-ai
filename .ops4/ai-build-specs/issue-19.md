...
## Objective
Implement a new public landing page at the root URL (/) with a modern, minimal card layout introducing the healthcare recruitment platform. Add a registration page (/register) using the same form and card patterns as the login page. Both pages must use only existing design system components and maintain visual and UX continuity with the current app.
...
## Requirements
- Create a new landing page at / using a centered card (max-w-md, p-8, rounded-lg, shadow-sm, bg-card, border-border) with the platform name, a concise tagline, and 2-3 feature highlights (bullets or icons) reflecting core platform capabilities.
- Add two primary action buttons to the landing page: 'Login' (primary, links to /login) and 'Register' (secondary, links to /register), using existing button styles.
- Do not render sidebar, topbar, or any authenticated navigation on the landing or registration pages.
- Create a new /register page with a registration form: email, password, confirm password (with show/hide toggle and password requirements tooltip), styled identically to the login form.
- Add a primary 'Register' button and a secondary 'Back to Login' link/button (links to /login) on the registration page.
- Implement error, loading, and plan_limit/permission_denied states on the registration page using the same Alert and Banner patterns as the login flow.
- On successful registration, redirect to /login or directly into the app as per existing authentication flow.
- Ensure both pages are fully responsive and visually consistent with the locked design system (typography, spacing, color, card/button/input styles, accessibility).
- Do not modify or redesign any existing dashboard, navigation, or authenticated pages.
...
## Acceptance Criteria
- Landing page at / displays platform name, tagline, feature highlights, and Login/Register buttons in a centered card with no sidebar/topbar.
- Login button links to /login; Register button links to /register.
- Registration page at /register uses the same card, input, and button styles as the login page, with all required fields and states.
- Registration form handles error, loading, and plan_limit/permission_denied states using existing Alert/Banner components.
- Successful registration redirects as per current authentication flow.
- Both pages are responsive and match the locked design system.
- Lint passes
- Build passes
...
`ai-build/issue-19-add-public-landing-and-registration-pages-with-a`
...
PR title should be:
AI Build: Add public landing and registration pages with auth links
